...
static void init_windows(void)
{
    ...
    window[TRACK] = newwin(max_y - STATUS_HEIGHT, max_x, 0, 0);
    window[SERIAL] = newwin(max_y, max_x, 0, 0);
    ...
    mvwprintw(window[HELP], 8, 2, "c   Increase code phase rate change");
    mvwprintw(window[HELP], 9, 2, "v   Decrease code phase rate change");
    ...
    panel[SERIAL] = new_panel(window[SERIAL]);
    ...
    set_panel_userptr(panel[EPHEMERIS], panel[SERIAL]);
    set_panel_userptr(panel[SERIAL], panel[TRACK]);

    // Print title of each window
    show_window(window[TRACK], "GPS Simulator Setup");
    show_window(window[LS_FIX], "GPS Simulation Status");
    show_window(window[EPHEMERIS], "CODES");
    show_window(window[SERIAL], "Serial");
...
void gui_gps_status_wprintw(status_color_t clr, const char *fmt, ...)
{
    ...
    if (clr > 0)
    {
        wattron(window[SERIAL], COLOR_PAIR(clr));
    }
    vw_printw(window[SERIAL], fmt, args);
    if (clr > 0)
    {
        wattroff(window[SERIAL], COLOR_PAIR(clr));
    }
    va_end(args);
    wrefresh(window[TOP]);
    pthread_mutex_unlock(&gui_lock);
